...
const App = () => {
    ...
    return (
        <ThemeProvider theme={getMuiTheme(theme)}>
            <React.Fragment>
                <CssBaseline />
                <TopBar appName="GridMergeApp" onParametersClick={() => console.log("onParametersClick")} onLogoutClick={() => logout(dispatch, userManager.instance)} onLogoClick={() => onLogoClicked()} user={user}/>
                { user !== null ? (
                        <Switch>
                            <Route exact path="/">
                                <Box mt={20}>
